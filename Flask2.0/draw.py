...
class Sketchpad(Canvas):
    def __init__(self, parent, **kwargs):
        ...
        self.bind("<Button-1>", self.release)
    ...
    def release(self, event):
        ...
        if self.closed:
            ...
            for i in l:
                newLoc = []
                loc = self.path[i]
                newLoc.append((loc[0] - (self.grid_size/2)) * (self.cutLength/self.grid_size))
                newLoc.append(((loc[1] - (self.grid_size/2)) * -1) * (self.cutLength/self.grid_size))
                self.path[i] = newLoc
            self.toolPaths.append(self.path)
            self.path = []
            self.cnt = 0
            self.closed = False 
        ...
        for path in self.toolPaths:
            ...
            for loc in path:
                ...
                if i == 0:
                    ...
                    self.gCode += "G01 X"+x+" Y"+y+" Z"+self.cutDepth+" F"+feed+"\n"
                    i += 1
